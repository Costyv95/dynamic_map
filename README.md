...
- **Room panel** — tapping a room (default `zoom` action) opens a glass panel with everything in the room's Home Assistant area: lights and switches as toggles, covers/locks/media/vacuum buttons, a climate stepper, sensor values. Set `room_panel: false` to turn it off, `room_panel_max: 20` to show more rows.
...
- **Room alerts** — a badge in a room's corner counts open doors/windows, leaks, smoke and unavailable devices of its area (amber; red for danger). Tap it to open the room. `room_alerts: false` turns it off.
