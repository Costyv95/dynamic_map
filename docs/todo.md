...
- [x] **Zoom smoothness** (2026-09-11): `animateViewBox` now animates a compositor transform on the `<svg>` (Web Animations API, `will-change: transform`) and writes the viewBox once at the end (2 writes per zoom, 0 long tasks measured on prod); effects loop capped at 30 fps and parked while the tab is hidden; presence dots stop writing once settled; the image "off" look uses opacity instead of a raster filter and treats `standby` as off.
- [x] **TV / projector badges** (2026-09-11): data fix on prod — `sc_living_room_tv` → `media_player.lg_tv`, `sc_living_room_projector` → `media_player.miproja1` (the old ids were dead/nonexistent, hence the permanent red strike). Backup: `shortcuts_floor2.json.bak-tvproj-*`.
- [x] **All lights off chip** (2026-09-11): built into `card/QuickActions.js`, count of lights on, two taps, hidden when none is on; `lights_off_button: false` removes it.
- [x] **Trend chart time axis** (2026-09-12): `card/TrendChart.js` — fixed 24 h window, round-hour ticks every 6 h + "now", hover/touch readout (`HH:MM · value`), panel re-render paused while a finger is on it.
